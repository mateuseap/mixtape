...
describe('tracks API', () => {
  it('uploads an mp3, lists it, streams it, then deletes it', async () => {
    ...
    const list = await request(app).get('/api/tracks').expect(200);
    expect(list.body).toHaveLength(1);

    await request(app).get(`/api/tracks/${id}/stream`).expect(200);

    await request(app).delete(`/api/tracks/${id}`).expect(204);
    const after = await request(app).get('/api/tracks').expect(200);
  ...
  it('returns 404 deleting a track that does not exist', async () => {
    await request(app).delete('/api/tracks/missing').expect(404);
  });
});
